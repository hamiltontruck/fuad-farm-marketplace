  ...
  return (
    <main className="marketplace-app">
        ...
        <div className="market-actions">
          ...
          {profile ? (
            <div className="profile-menu-wrap">
              ...
              {profileMenuOpen && <div className="profile-popover"><span>{profileRole?.icon ?? "👤"}</span><div><strong>{profile.fullName}</strong><small>{profileRole ? getRoleName(profileRole, language) : profile.role}</small></div><button type="button" onClick={logout}>{t.logout}</button></div>}
            </div>
          ) : (
          ...
          <button className="post-ad-button ripple" type="button" onClick={() => setPostOpen(true)}><span>＋</span> {t.postAd}</button>
        ...
        {mobileMenuOpen && (
          <nav className="mobile-market-menu" aria-label={t.mobileNav}>
            ...
            <div className="mobile-account-actions">
              {profile ? <><div><span>{profileRole?.icon ?? "👤"}</span><strong>{profile.fullName}</strong></div><button type="button" onClick={logout}>{t.logout}</button></> : <><button type="button" onClick={() => { setMobileMenuOpen(false); setLoginOpen(true); }}>{t.login}</button><button className="mobile-register" type="button" onClick={() => startRegistration()}>{t.register}</button></>}
            </div>
          </nav>
        )}
      </header>
      ...
      <section className="final-cta">
        <div><span>FUAD ESMART TRADING PLC</span><h2>{t.ctaTitle}</h2><p>{t.ctaBody}</p></div><div><button className="cta-register ripple" type="button" onClick={() => startRegistration()}>{t.createAccount}</button><button className="cta-post ripple" type="button" onClick={() => setPostOpen(true)}>＋ {t.postFree}</button></div>
      </section>
      ...
      <footer className="market-footer">
        ...
        <div><strong>{t.marketplace}</strong><a href="#categories">{t.categories}</a><a href="#listings">{t.latestAds}</a><button type="button" onClick={() => setPostOpen(true)}>{t.postAd}</button></div>
      ...
      {postOpen && <PostAdFlow open language={language} onClose={() => setPostOpen(false)} onComplete={addListing} defaultSeller={profile?.fullName ?? ""} />}
